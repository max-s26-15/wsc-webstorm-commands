...
- Перед запуском команди завжди друкуються одним викликом `log.warn` (stderr; `warn`, щоб пережити `WSC_LOG_LEVEL=warn`): заголовок `custom commands from the preset:`, далі рядки `  <name>: <commands joined>`, з'єднані `\n`.
